...
- **Debugging note:** `ros2 topic list` is unreliable in this image — FastDDS shared memory is disabled, so discovery times out and live, actively-published topics go missing from the listing. Use `ros2 node info /<node>`, which asks the node directly. This briefly looked like the follower's error topics had never been created; they had
